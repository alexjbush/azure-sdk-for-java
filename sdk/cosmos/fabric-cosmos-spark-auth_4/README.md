...
This project provides an implementation of the `AccountDataResolver` interface for the Azure Cosmos DB Spark 4 Connector, specifically designed to work with Azure Fabric accounts. 
...
[AAD Auth Documentation](https://github.com/Azure/azure-sdk-for-java/tree/main/sdk/cosmos/azure-cosmos-spark_3/docs/AAD-Auth.md) (the Spark 3 documentation applies equally to Spark 4). Fabric Native accounts only support gateway mode, so the `spark.cosmos.useGatewayMode` must be set to `true` in the Spark configuration for these accounts.
...
* There is no support for the [catalog api](https://github.com/Azure/azure-sdk-for-java/tree/main/sdk/cosmos/azure-cosmos-spark_3/docs/catalog-api.md) operations (see Spark 3 documentation) using this implementation of the `AccountDataResolver` with a CosmosDB Fabric Native Account.
